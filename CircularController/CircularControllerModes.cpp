...
void setSparkle() {
  program_sparkle_fmt(rfm69.send_buffer, rfm69.send_data_size,
                      config.address, find_output_type(HMTL_OUTPUT_PIXELS),
                      100, 0);
  handler.process_msg((msg_hdr_t *)rfm69.send_buffer, &rfm69, NULL, &config);
}

void setBlink() {
  hmtl_program_blink_fmt(rfm69.send_buffer, rfm69.send_data_size,
                         config.address, find_output_type(HMTL_OUTPUT_PIXELS),
...
void setCancel() {
  hmtl_program_cancel_fmt(rfm69.send_buffer, rfm69.send_data_size,
                          config.address, find_output_type(HMTL_OUTPUT_PIXELS));
  handler.process_msg((msg_hdr_t *)rfm69.send_buffer, &rfm69, NULL, &config);
}
